...
rand_stones(Farm.object_list, Img_Stone, 20)
...
while done == False:
        ...
        if Worker.map_mode == 'mode_base':
            if event.type == pygame.MOUSEMOTION:
                ...
                if Pane_Map.IsOn(mouse_pos):
                    Worker_Output = Farm.Activate(mouse_pos)
            ...
            if event.type == pygame.MOUSEMOTION:
                ...
                if Farm.can_build(mouse_pos) and Pane_Map.IsOn(mouse_pos):
                    Farm.add(mouse_pos)
                else:
                    Worker_Output = CheckAll_And_Action(ACTIVE_PANES, BUTTON_DICT, mouse_pos)
                    if Worker_Output is not None:
    ...
    if  Worker.map_mode == 'mode_constructing' and hasattr(Worker.item, 'img') and Pane_Map.IsOn(mouse_pos):
        ...
        if Farm.can_build(mouse_pos):
            Worker.item.img.draw(pos_x, pos_y, [2,2])
        else:
            Worker.item.img.draw(pos_x, pos_y, [2,2], [0,1])
